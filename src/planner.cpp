...
void Planner::create_trajectory(Map& map, Road& road, Vehicle& car, vector<vector<double>>& trajectory) {
  ...
  if (current_points < POINTS) {
	...
	// if previous action was to change lane (left or right)
	else {
  ...
  // if we did generate new points, estimate new points (XY)
  if (this->new_points) {
...
void Planner::stay_in_lane(Vehicle& car){
  ...
  this->n = CYCLES*POINTS; // plan 2 cycles ahead (100 time steps)
  double target_v = min(car.prev_s()[1]*1.3, SPEED_LIMIT); // speed up by 30% or if speed limit already reached stay
  double target_s = car.prev_s()[0] + n * AT * target_v; // 400 m = 100 timesteps * 0.2 sec / timestep * 20 m / sec)

  this->start_s = {car.prev_s()[0], car.prev_s()[1], car.prev_s()[2]}; // s, s_dot = v, s_double_dot = a
  this->end_s = {target_s, target_v, 0.0}; // s, s_dot = v, s_double_dot = a

  double target_d = get_lane_d(car.prev_d()[0]);

  this->start_d = {get_lane_d(car.prev_d()[0]), 0.0, 0.0}; // d, d_dot, d_double_dot
  this->end_d = {target_d, 0.0, 0.0}; // d, d_dot, d_double_dot
...
void Planner::reduce_speed(Vehicle& car){
  ...
  this->n = CYCLES*POINTS; // plan 2 cycles ahead (100 time steps)
  this->new_points = true;
  double target_v = max(car.prev_s()[1]*0.8, SPEED_LIMIT/2); // reduce speed to 10 m/s or 80% of previous speed if that was slower
  double target_s = car.prev_s()[0] + n * AT * target_v; // 400 m = 100 timesteps * 0.2 sec / timestep * 20 m / sec)
...
void Planner::change_lane(Vehicle& car, LANE target_lane){
  cout << "ACTION: change lane" << endl;
  this->n = CYCLES*POINTS; // plan 2 cycles ahead (100 time steps)
  this->new_points = true;
  double target_v = car.prev_s()[1]; // stay the same speed
  double target_s = car.prev_s()[0] + n * AT * target_v; // 400 m = 100 timesteps * 0.2 sec / timestep * 20 m / sec)
