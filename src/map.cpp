...
using namespace std;
using namespace tk; // using this namespace for spline
...
Map::Map(string map_file_) {
  ...
  // create 4 splines spline with s-axis and x,y,dx,dy respectively
...
vector<double> Map::getXY(double s, double d){
  ...
  // get waypoint for given s based on spline interpolation
  wp_x = this->wp_spline_x(s);
  wp_y = this->wp_spline_y(s);
  wp_dx = this->wp_spline_dx(s);
  wp_dy = this->wp_spline_dy(s);

  // using spline interpolation, calc next waypoint
  next_x = wp_x + wp_dx * d;
  next_y = wp_y + wp_dy * d;
